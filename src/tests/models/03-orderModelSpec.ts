...
describe('Order Model Testing', async () => {
    ...
    describe('Basic Order Modification Tests', () => {
        ...
        it('addProduct method of the model should fail to add products due to adding the same product again', async () => {
            await expectAsync(store.addProduct({order_id: 1, product_id: 2, quantity: 1})).toBeRejected();
        })

        it('addProduct method of the model should fail to add products to a complete order', async () => {
            await expectAsync(store.addProduct({order_id: 2, product_id: 2, quantity: 1})).toBeRejected();
        })
    ...
    describe('delete method of the model should delete the order and its products', async () => {
        await store.delete({id: 1})
        const result = await store.read('*')
        const orderProd = await dash.readOrderProducts(1)
        it('order should be deleted from the orders table', () => {
            expect(result).toEqual([
                {
                    id: 2,
                    user_id: 2,
        ...
        it('order products should be deleted from the orders_products table', () => {
